...
for i = 1:c
  ...
  ttCreateMailbox(['mbA_temp' num2str(vid)], sw_buflen);
end

recv_task = 'sA_TSK_Recv';
send_task = 'sA_TSK_SND';

ttCreateTask(recv_task, 10, recv_task);
ttCreateTask(send_task, 10, send_task);

ttSetPriority(1, recv_task);
% 调度的优先级比Recv优先级低（调度在接收之后）
ttSetPriority(10, send_task);

% 所有接收信息的网络号
ttAttachNetworkHandler(1, recv_task);
ttAttachNetworkHandler(2, recv_task);

% ttNoSchedule(recv_task);
% ttAttachNetworkHandler(2, recv_task);
