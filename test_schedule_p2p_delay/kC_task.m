    ...
    case 2
        ...
        if ~isempty(msg)
            ...
            if(msg.vid==3)
                ...
                plot_data.from=msg.from;
                plot_data.destination='kC';
                v3_delay_array(end+1)=plot_data;
            end
            if(msg.vid==4)
                plot_data.delay=delay;
                plot_data.curTime=ttCurrentTime;
                plot_data.from=msg.from;
                plot_data.destination='kC';
                v4_delay_array(end+1)=plot_data;
            end
        else
            delay = 0;
        end
        ...
        if ttCurrentTime>=sim_end_time && file_flag_kC==false
            ...
            struct2file(v3_delay_array,'v3_delay_array.xlsx');
            struct2file(v4_delay_array,'v4_delay_array.xlsx');
            file_flag_kC = true;
        end
        exectime = -1;
end
